...
const LEVEL_COLORS: Record<string, string> = {
  "Pre-A1": "bg-emerald-100 text-emerald-700 border-emerald-300",
  "A1": "bg-blue-100 text-blue-700 border-blue-300",
  "A2": "bg-amber-100 text-amber-700 border-amber-300",
  "B1": "bg-rose-100 text-rose-700 border-rose-300",
};

const DIFFICULT_LEVELS = new Set(["A1", "A2", "B1"]);
...
export function WordExplorer() {
  ...
  // TTS state for sentence playback
  ...
  const handleWordClick = useCallback(async (word: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("word-enrichment", {
        body: { word, context: sentence || undefined },
      });
      ...
      if (payload.level) {
        setWordLevels((prev) => ({ ...prev, [word.toLowerCase()]: payload.level }));
      }
    } catch (err: any) {
  ...
  }, [activeWord, sentence, toast]);

  // ── Azure TTS for example sentences ──
  const playExampleSentence = useCallback(async (text: string, index: number) => {
    setPlayingSentence(index);
    try {
      const { data, error } = await supabase.functions.invoke("pronounce-viseme", {
        body: { word: text, sentence: true },
      });

      if (error || data?.error || data?.fallback) {
        // Fallback to browser TTS
        speak(text, 0.85);
        setTimeout(() => setPlayingSentence(null), 3000);
        return;
      }
          ...
          <CardContent className="p-5 sm:p-6 space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <Badge
                      ...
                      className={cn(
                        ...
                        LEVEL_COLORS[enrichment.level] || "bg-slate-100 text-slate-600"
                      )}
                    >
              ...
              <div className="flex items-center gap-2">
                <VisemePlayer word={enrichment.root_word} compact />
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
                  onClick={() => speak(enrichment.root_word, 0.7)}
                  className="rounded-xl gap-1.5 text-violet-600"
                >
            ...
            {/* ── Word Forms ── */}
            {enrichment.word_forms.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                  📝 Word Forms
                </p>
                <div className="flex flex-wrap gap-1.5">
                  {enrichment.word_forms.map((form, i) => (
                    <button
                      key={i}
                      type="button"
                      onClick={() => speak(form)}
                      className="px-3 py-1.5 rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-200 text-sm font-medium hover:bg-violet-200 dark:hover:bg-violet-800/60 transition-colors"
                    >
                      {form}
                    </button>
                  ))}
            ...
            {/* ── Example Usages with Vietnamese ── */}
            ...
            {/* ── Viseme + Webcam Mirror ── */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                  👄 Mouth Shapes
                </p>
                <VisemePlayer word={enrichment.root_word} />
              </div>
              <WebcamMirror />
            </div>
